...
def create_user(db, config, firstname, lastname, username, password):
    ...
    cursor = db.cursor()
    cursor.execute(
        'INSERT INTO users(firstname, lastname, created_at, is_admin) values(%s, %s, CURRENT_TIMESTAMP, FALSE) RETURNING id;',
        (firstname, lastname))
...
def user_id_by_username(db, username):
    ...
    cursor = db.cursor()
    cursor.execute("SELECT user_id from auth_methods where username=%s and type='USERNAME_AND_PASSWORD'", (username,))
    row = cursor.fetchone()
    if row:
        user_id = int(row["user_id"])
